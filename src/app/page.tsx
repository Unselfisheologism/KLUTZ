...
function ChatComponent({ messages, setMessages, currentChatId, setCurrentChatId }: {
  ...
  const handleSendMessage = async (messageText: string) => {
    ...
    if (currentChatId === null) {
        const newChatId = `chat-${Date.now()}`; // Simple timestamp-based ID
    ...
    try {
      ...
      const data = await response.json();
       if (response.ok) {
            ...
            // Handle API errors (e.e.g., website not found, server error)
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (currentChatId && messages.length > 0) {
      // Generate title from the first 4 words of the AI's first response
      const firstBotMessage = messages.find(msg => msg.sender === 'bot');
      const title = firstBotMessage ? firstBotMessage.text.split(' ').slice(0, 4).join(' ') : 'New Chat';
  ...
  }, [messages, currentChatId]); // Save whenever messages or currentChatId change
            ...
            <div className="p-4">
              ...
              <ul>
                {chatSessions.map((session) => (
                  <li key={session.id} className="mb-2" key={session.id}>
                    <button
